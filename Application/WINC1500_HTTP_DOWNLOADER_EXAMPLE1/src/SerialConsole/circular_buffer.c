...
* @author	   Phillips Johnston
* @date        Aug 6, 2018
* @version	   0.2   ///<Additional circular buffer functions added by Tushar G. Hedaoo on Feb 7, 2019
*****************************************************************************/
 #include <stdlib.h>
 #include <stdint.h>
 #include <stddef.h>
 ...
 void circular_buf_put(cbuf_handle_t cbuf, uint8_t data)
{
 ...
 /* Backspace implementation directly acting on the cbuf*/
 void circular_buf_backspace(cbuf_handle_t cbuf)
 {
		if(cbuf->head != 0 && cbuf->head != 1)
		{
			cbuf->head = (cbuf->head - 2) % cbuf->max;	
		}
		else if (cbuf->head == 1)
		{
			cbuf->head = (cbuf->head - 1) % cbuf->max;
